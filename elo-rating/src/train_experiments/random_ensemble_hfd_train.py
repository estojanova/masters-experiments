...
ex = Experiment(name="random_ensemble_hfd_train")
...
def play_pair(_rnd, x, learner1: RandomLearner, learner2: RandomLearner):
...
def train_pair(x, y, learner1: RandomLearner, learner2: RandomLearner):
    prediction1 = learner1.model.predict_one(x)
    prediction2 = learner2.model.predict_one(x)
...
def test_ensemble(_run, test_set, ensemble: list[RandomLearner], step_nr, nr_samples_test):
...
def train_ensemble(_run, _rnd, ensemble: list[RandomLearner], train_set, test_set, nr_samples_train, nr_samples_test,
                   test_step, pick_train_pairs_strategy, pick_play_pairs_strategy, nr_pairs, repeat):
...
def run(_run, _seed, meta_experiment, train_data_set, test_data_set, nr_samples_train, nr_samples_test, test_step,
    ...
    random.seed(_seed)
    ensemble = list(RandomLearner(i, tree.HoeffdingTreeClassifier()) for i in range(nr_learners))
