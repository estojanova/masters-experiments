...
ex = Experiment(name="single_vs_accuracy_ensemble_random_tree_label_count")
...
def collect_metrics(meta_uuid, _run):
    ...
    try:
            ...
            for sub_run in sub_runs:
                ...
                if sub_run_name == "accuracy_ensemble_hfd_train":
                    majority_test_accuracy = \
                        metrics.find({"run_id": sub_run_id, "name": "ensemble.majority_test_accuracy"})[0].get("values")
                    majority_validation_accuracy = \
                        metrics.find({"run_id": sub_run_id, "name": "ensemble.majority_validation_accuracy"})[0].get(
                            "values")
        ...
        for index, value in enumerate(avg_majority_validation_accuracy.tolist()):
            _run.log_scalar("acc_ensemble.avg_majority_val_accuracy", value, index + 1)
...
def run(_run, _seed, meta_experiment, n_num_features, n_cat_features, n_categories_per_feature, max_tree_depth,
    ...
    for count in range(0, nr_runs_per_config):
        ensemble_training_exp.add_config(
            ...
            train_data_set=train_set_mask,
            test_data_set=test_set_adapted,
            validation_data_set=validation_set_adapted,
            nr_samples_train=nr_samples_train,
            nr_samples_test=nr_samples_test,
