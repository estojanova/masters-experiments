...
ex = Experiment(name="acc_no_play_vs_acc_all_play_sea_label_count")
    ...
    try:
        with MongoClient(uri) as client:
            ...
            for sub_run in sub_runs:
                ...
                majority_accuracy = metrics.find({"run_id": sub_run_id, "name": "ensemble.majority_validation_accuracy"})[
                    0].get("values")
            ...
            for index, value in enumerate(average_majority_accuracy.tolist()):
                _run.log_scalar("acc_ensemble.{}_play.avg_majority_accuracy".format(play_pairs_strategy), value, index + 1)
